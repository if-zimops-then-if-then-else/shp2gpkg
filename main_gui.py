...
def main_single(sog_file, par_file, ttc_file, shp_file):
    print("STARTING")
    time.sleep(1)
    # as long as there are more files to work with loop through the process
    # check for the next files and move them to the working directory
...
def main():
    ...
    def get_user_input(path):
        if lb.get(lb.curselection()) == 'single':
            main_single()
        elif lb.get(lb.curselection()) == 'all':
            main_loop()
        else:
    ...
    def select_folder():
        ...
        if folder_path:
            folder_path_entry.delete(0, tk.END)  # Clear any previous entry
            folder_path_entry.insert(0, folder_path)  # Display the selected folder path

    frames = []  # List to hold all frames
    current_frame_index = 0  # Index of the current frame

    frame1 = ttk.Frame(root)
    lb = tk.Listbox(frame1, listvariable=choicesvar)
    frame2 = ttk.Frame(root)
    folder_path_entry = ttk.Entry(frame2, width=40)
    folder_path_entry.grid(row=0, column=0, padx=10, pady=10, columnspan=2)

    # Create a button to open the file dialog
    select_button = ttk.Button(frame2, text="Select Folder for Data", command=select_folder)
    select_button.grid(row=1, column=0, padx=10, pady=10)

    # Create a button to perform an action with the selected folder path (you can replace this with your specific action)
    action_button = ttk.Button(frame2, text="Start", command=lambda: get_user_input(folder_path_entry.get()))
    action_button.grid(row=1, column=1, padx=10, pady=10)

    current_frame = frames[current_frame_index]
    current_frame.pack()
    start_button = ttk.Button(frame1, text='Continue', command=show_next_frame)
    back_button = ttk.Button(frame2, text='Back', command=show_previous_frame)
